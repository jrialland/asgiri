...
@pytest.mark.parametrize("protocol_version", [HttpProtocolVersion.HTTP_1_1, HttpProtocolVersion.AUTO])
...
async def test_post_echo_endpoint(
    ...
    response = httpx.post(f"http://127.0.0.1:{unused_port}/echo", json={"message": "Hello, World!"})
    assert response.status_code == 200
...
@pytest.mark.timeout(5)
@pytest.mark.asyncio
async def test_get_with_parameters(protocol_version: HttpProtocolVersion, unused_port: int):
    server = Server(
        app=app, host="127.0.0.1", port=unused_port, http_version=protocol_version
    )
    ...
    # Make a request to the /helloworld endpoint
    response = httpx.get(f"http://127.0.0.1:{unused_port}/read_params?name=John&age=30&active=true")
    assert response.status_code == 200
...
async def test_http2_direct_connection(unused_port: int):
    ...
    server = Server(
        app=app, host="127.0.0.1", port=unused_port, http_version=HttpProtocolVersion.AUTO
    )
...
async def test_protocol_detection_http11(unused_port: int):
    ...
    request = b"GET /helloworld HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\n\r\n"
...
async def test_protocol_detection_http2(unused_port: int):
    ...
    settings_received = any(isinstance(e, h2.events.RemoteSettingsChanged) for e in events)
...
async def test_http2_with_httpx_client(unused_port: int):
    ...
    server = Server(
        app=app, host="127.0.0.1", port=unused_port, http_version=HttpProtocolVersion.HTTP_2
    )
