...
class WebSocketProtocol:
    ...
    def __init__(
        ...
        if raw_request:
            ...
            self.logger.debug("HTTP/2 WebSocket - simulated handshake for wsproto state")
        
        # Queues for ASGI communication
        self.receive_queue: asyncio.Queue[WebSocketReceiveEvent | WebSocketDisconnectEvent] = asyncio.Queue()
        self.send_queue: asyncio.Queue[WebSocketAcceptEvent | WebSocketSendEvent | WebSocketCloseEvent] = asyncio.Queue()
    ...
    async def _send(self, message: WebSocketAcceptEvent | WebSocketSendEvent | WebSocketCloseEvent):
        """ASGI send callable.
    ...
    async def _handle_accept(self, message: WebSocketAcceptEvent):
        ...
        if self.needs_handshake_response:
            ...
            # Make sure extra_headers is a list, not None
            ...
            self.logger.debug(f"WebSocket handshake sent with subprotocol: {subprotocol}")
    ...
    async def _handle_event(self, event: Event):
        ...
        if isinstance(event, Request):
            ...
            self.logger.debug(f"WebSocket close from client: {event.code} {event.reason}")
            ...
            if not self.closed:
                data = self.ws.send(CloseConnection(code=event.code, reason=event.reason))
                self.transport.write(data)
                self.closed = True
    ...
    def close(self, code: int = 1000, reason: str = ""):
        ...
        asyncio.create_task(self.receive_queue.put({
            "type": "websocket.disconnect",
            "code": code,
        }))
